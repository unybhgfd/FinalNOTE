...
**经验风险最小化**：寻找代价函数的最小值，代价函数一般写作：

$$
J(\bm \theta) =
\mathbb{E}_{(\mathbf x, \mathrm y) \sim p_\text{data}}
L(f(\bm x; \bm \theta), y)
$$
...
早停法使用的代价函数需要是原始的代价函数，比如 0-1 损失。

### 小批量（minibatch）

由于训练集可能很庞大，直接计算整个训练集上的梯度不现实，于是我们选 $n$ 个样本进行计算。$n$ 被称为批量大小。选取方法一般是每 epoch 先打乱样本，然后每次随机抽 $n$ 个计算梯度，被选中的样本下次不会再选择。

**epoch**：数据集中的所有样本都被抽取计算过梯度，这个过程就是一个 epoch。

**batch**：计算 $n$ 个样本梯度并更新参数的过程。，这里的 $n$ 就是 **batch size**。
